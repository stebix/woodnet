...
class ResNet3D(torch.torch.nn.Module):
    ...
    block : Type[ResNetBlock]
        Type of the basic block to use inside the ResNet model.

    final_nonlinearity : str
        Name of the final nonlinearity to apply to the output.
        Available are:
    ...
    final_nonlinearity_kwargs : dict | None
        Keyword arguments for the final nonlinearity.
        Will be passed through to the `torch.nn.Module` constructor.

    testing : bool
        Flag to indicate if the model is used for testing.
        If set to `True`, the final nonlinearity will be applied
    ...
    num_classes : int
        Class variable. Number of output neurons and thus classes.
    
    num_layers : int
        Class variable. Number of layers in the ResNet model.

    layers : list[int]
        Class variable. Number of blocks per layer in the ResNet model.

    expansion : int
        Class variable. Expansion factor for the perceptron.

    conv_1_channels : int
        Class variable. Number of output channels of the first convolution.

    channel_cascade : list[int]
        Class variable. Number of channels per layer in the ResNet model.

    
    Notes
    -----
    Foundation by `He et al.`: https://arxiv.org/abs/1512.03385
    """
