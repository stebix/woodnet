...
class BaseTileDatasetBuilder:
    """
    Build 3D TileDataset instances programmatically.

    Parameters
    ----------

    dataset_class : type
        Class object of the dataset that is build.

    instances_ID : Iterable[str]
        Iterable of instance IDs to build datasets for.
        Must be present in the data configuration
        and instance mapping hierarchy.

    phase : Literal['train', 'val', 'test']
        Phase of the dataset to build.

    tileshape : Tileshape3D
        Shape of the 3D tiles to extract from the volume data.

    transform_configurations : Iterable[dict] | None, optional
        Iterable of transformation configurations to apply to the data.
    ...
    datasets : list[torch.utils.data.Dataset]
        List of built dataset instances.
    ...
    instance_mapping : dict[str, InstanceFingerprint]
        Class variable. Mapping of instance IDs to instance
        fingerprints.

    classlabel_mapping : dict[str, int]
        Class variable. Mapping of human-readable class names
        to integer labels.

    internal_path : str
        Class variable. Path within the dataset file to
        the actual data.

    pretty_phase_name_map : dict[str, str]
        Class variable. Mapping of abbreviated phase names to
        nicer human-readable phase names.
    """
...
class TileDatasetBuilder(BaseTileDatasetBuilder):
    """
    Builder specialized for the standard TileDataset for training, validation and testing.
    """
    def build(cls,
              instances_ID: Iterable[str],
        ...
        """
        Build a list of TileDataset instances for the given instances,
        phase and tileshape.
        ...
        datasets : list[TileDataset]
            List of built TileDataset instances.
        """
