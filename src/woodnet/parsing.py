...
# separate global definition of the description strings to synchronize 
# actual terminal string with sphinx helper code
VERB_TRAIN_DESC = 'Perform single configuration training.'
VERB_BATCHTRAIN_DESC = 'Perform batched multi-configuration training.'
VERB_PREDICT_DESC = 'Perform prediction experiment.'
VERB_EVALUATE_DESC = 'Perform evaluation experiment.'
...
def configure_predict_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """In-place modify the `parser` to work as a prediction run parser."""
    parser.add_argument('configuration', type=str,
                        help='Location of the configuration YAML file that fully defines '
                             'the prediction experiment run.')
    configure_globopts(parser)
    return parser
...
def create_parser() -> argparse.ArgumentParser:
    ...
    train_parser = subparser.add_parser('train', help=VERB_TRAIN_DESC)
    batchtrain_parser = subparser.add_parser('batchtrain', help=VERB_BATCHTRAIN_DESC)
    predict_parser = subparser.add_parser('predict', help=VERB_PREDICT_DESC)
    evaluate_parser = subparser.add_parser('evaluate', help=VERB_EVALUATE_DESC)
...
############### Note: The following code is only used for sphinx documentation generation ####################

def _make_evaluate_parser() -> argparse.ArgumentParser:
    """
    Create the evaluation experiment parser.
    This is done for sphinx_argparse_cli to be able to generate the documentation.
    """
    parser = argparse.ArgumentParser(description=VERB_EVALUATE_DESC)
...
def _make_train_parser() -> argparse.ArgumentParser:
    """
    Create the single training experiment parser.
    This is done for sphinx_argparse_cli to be able to generate the documentation.
    """
    parser = argparse.ArgumentParser(description=VERB_TRAIN_DESC)
    configure_train_parser(parser)
    return parser


def _make_batchtrain_parser() -> argparse.ArgumentParser:
    """
    Create the batch training experiment parser.
    This is done for sphinx_argparse_cli to be able to generate the documentation.
    """
    parser = argparse.ArgumentParser(description=VERB_BATCHTRAIN_DESC)
    configure_batchtrain_parser(parser)
    return parser


def _make_predict_parser() -> argparse.ArgumentParser:
    """
    Create the prediction experiment parser.
    This is done for sphinx_argparse_cli to be able to generate the documentation.
    """
    parser = argparse.ArgumentParser(description=VERB_PREDICT_DESC)
    configure_predict_parser(parser)
    return parser